...
function GalleryPage() {
  ...
  const handleClosePopUp = () => {
    callSorter(sortType); // this pretty much refreshes the gallery in case the user likes within the popup. updates the icon and like count
    setShowPopUp(false);
  };
  ...
  const callSorter = async (type) => {
    setSortType(type);
    var comp = version ? version : ""; // if in a comp, pass in comp id, else it is empty for no compettion
    let body = await sortImages(type, comp, -1); // type is sort type, comp is competition id, -1 is for amount which returns all
    if (!body) {
      return; // receive nothing from API call then return
    }
    let likedImages = [];
    let flag = true // flag to determine whether user is able to enter competition or not
    for (let comp = 0; comp < body.length; i++) {
      if (body[i]["liked_by_user"] == true) {
        likedImages.push(body[i]["drawing_id"]);
      }
      try { // check if user has already entered a submission
        if (
          body[i]["username"] ==
